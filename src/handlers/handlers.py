...
class Handlers:
    ...
    async def handle(self, operation, sub_operation, params, effective_user=None):
        handler = self.dispatch.get((operation, sub_operation))
        if not handler:
            logger.error(f"Unknown operation: {operation}/{sub_operation}")
            return {
                "status": "error",
                "message": f"Unknown operation: {operation}/{sub_operation}",
            }
        return await handler(params, effective_user=effective_user)

    @handle_errors
    async def _handle_query_server(self, params, effective_user=None):
    ...
    @handle_errors
    async def _handle_query_workspaces(self, params, effective_user=None):
        ...
        if not params.workspace_name and params.action in ["get", "type", "status"]:
            ...
            raise ValueError(
                f"workspace_name is required for this {params.action} action"
            )

        if params.action == "get":
            result = await self.workspace_services.get_workspace(
                params.workspace_name, effective_user=effective_user
            )
        elif params.action == "list":
            result = await self.workspace_services.list_workspaces(
                params.user, params.max_results, effective_user=effective_user
            )
        elif params.action == "type":
            result = await self.workspace_services.get_workspace_type(
                params.workspace_name, effective_user=effective_user
            )
        elif params.action == "status":
            result = await self.workspace_services.get_workspace_status(
                params.workspace_name, effective_user=effective_user
            )
        else:
    ...
    @handle_errors
    async def _handle_query_files(self, params, effective_user=None):
        if params.action == "content":
            result = await self.file_services.get_file_content(
                params.file_path, effective_user=effective_user
            )
        elif params.action == "history":
            result = await self.file_services.get_file_history(
                params.file_path, params.max_results, effective_user=effective_user
            )
        elif params.action == "info":
            result = await self.file_services.get_file_info(
                params.file_path, effective_user=effective_user
            )
        elif params.action == "metadata":
            result = await self.file_services.get_file_metadata(
                params.file_path, effective_user=effective_user
            )
        elif params.action == "diff":
            result = await self.file_services.diff_files(
                params.file_path,
                params.file2,
                params.diff2,
                effective_user=effective_user,
            ...
            result = await self.file_services.get_file_annotations(
                params.file_path, effective_user=effective_user
            )
        else:
    ...
    @handle_errors
    async def _handle_query_changelists(self, params, effective_user=None):
        if params.action == "get":
            ...
            result = await self.changelist_services.get_changelist(
                params.changelist_id, effective_user=effective_user
            )
        elif params.action == "list":
    ...
    @handle_errors
    async def _handle_query_shelves(self, params, effective_user=None):
        if not params.changelist_id and params.action in ["diff", "files"]:
            ...
            raise ValueError(
                f"changelist_id is required for this {params.action} action"
            )
        if params.action == "list":
            result = await self.shelve_services.list_shelves(
                params.user, params.max_results, effective_user=effective_user
            )
        elif params.action == "diff":
            ...
            result = await self.shelve_services.get_shelve_files(
                params.changelist_id, effective_user=effective_user
            )
        else:
    ...
    @handle_errors
    async def _handle_query_jobs(self, params, effective_user=None):
        ...
        if params.action == "list_jobs":
            result = await self.job_services.list_jobs_from_changelist(
                params.changelist_id, params.max_results, effective_user=effective_user
            )
        elif params.action == "get_job":
            result = await self.job_services.get_job_details(
                params.job_id, effective_user=effective_user
            )
        else:
    ...
    async def _handle_modify_workspaces(self, params, effective_user=None):
        ...
        if params.action == "create":
            result = await self.workspace_services.create_workspace(
                {k: v for k, v in params.specs.model_dump().items() if v is not None},
                effective_user=effective_user,
            )
        elif params.action == "delete":
            result = await self.workspace_services.delete_workspace(
                params.name, effective_user=effective_user
            )
        elif params.action == "update":
            result = await self.workspace_services.update_workspace(
                params.name,
                {k: v for k, v in params.specs.model_dump().items() if v is not None},
                effective_user=effective_user,
            )
        elif params.action == "switch":
            result = await self.workspace_services.switch_workspace(
                params.name, effective_user=effective_user
            )
        else:
    ...
    @handle_errors
    async def _handle_modify_files(self, params, effective_user=None):
        if not params.file_paths and params.action in [
            "add",
            "edit",
            "delete",
            "revert",
        ...
        if params.action == "add":
            result = await self.file_services.add_files(
                params.file_paths, params.changelist, effective_user=effective_user
            )
        elif params.action == "edit":
            result = await self.file_services.edit_files(
                params.file_paths, params.changelist, effective_user=effective_user
            )
        elif params.action == "move":
            if not params.source_paths or not params.target_paths:
                raise ValueError(
                    "source_paths and target_paths required for move action"
                )
            result = await self.file_services.move_files(
                params.source_paths,
                params.target_paths,
                params.changelist,
                effective_user=effective_user,
            ...
            result = await self.file_services.delete_files(
                params.file_paths, params.changelist, effective_user=effective_user
            )
        elif params.action == "revert":
            result = await self.file_services.revert_files(
                params.file_paths, params.changelist, effective_user=effective_user
            )
        elif params.action == "reconcile":
            result = await self.file_services.reconcile_files(
                params.file_paths or [],
                params.changelist,
                effective_user=effective_user,
            )
        elif params.action == "resolve":
            result = await self.file_services.resolve_files(
                params.file_paths or [],
                params.changelist,
                params.mode,
                effective_user=effective_user,
            ...
            result = await self.file_services.sync_files(
                params.file_paths, params.force, effective_user=effective_user
            )
        else:
    ...
    @handle_errors
    async def _handle_modify_changelists(self, params, effective_user=None):
        if not params.changelist_id and params.action in [
            "update",
            "submit",
            "delete",
            "move_files",
        ...
        if params.action == "create":
            result = await self.changelist_services.create_changelist(
                params.description, effective_user=effective_user
            )
        elif params.action == "update":
            result = await self.changelist_services.update_changelist(
                params.changelist_id, params.description, effective_user=effective_user
            )
        elif params.action == "submit":
            result = await self.changelist_services.submit_changelist(
                params.changelist_id, effective_user=effective_user
            )
        elif params.action == "delete":
            result = await self.changelist_services.delete_changelist(
                params.changelist_id, effective_user=effective_user
            )
        elif params.action == "move_files":
            if not params.file_paths:
                raise ValueError("file_paths required for move_files action")
            result = await self.changelist_services.move_files_to_changelist(
                params.changelist_id, params.file_paths, effective_user=effective_user
            )
        else:
    ...
    @handle_errors
    async def _handle_modify_shelves(self, params, effective_user=None):
        if not params.changelist_id:
        ...
        if params.action == "shelve":
            ...
            result = await self.shelve_services.shelve_files(
            ...
            result = await self.shelve_services.unshelve_files(
                params.changelist_id,
                params.file_paths,
                params.force,
                effective_user=effective_user,
            ...
            result = await self.shelve_services.update_shelve(
                params.changelist_id,
                params.file_paths,
                params.force,
                effective_user=effective_user,
            ...
            result = await self.shelve_services.delete_shelve(
                params.changelist_id, params.file_paths, effective_user=effective_user
            )
        elif params.action == "unshelve_to_changelist":
            result = await self.shelve_services.unshelve_to_changelist(
                params.changelist_id,
    ...
    @handle_errors
    async def _handle_modify_jobs(self, params, effective_user=None):
        if (
            not params.changelist_id
            and not params.job_id
            and params.action in ["link_job", "unlink_job"]
        ):
            logger.error(
                f"changelist_id and job_id are required for this {params.action} action"
            )
            raise ValueError(
                f"changelist_id and job_id are required for this {params.action} action"
            )
        if params.action == "link_job":
            result = await self.job_services.link_job_to_changelist(
                params.changelist_id, params.job_id, effective_user=effective_user
            )
        elif params.action == "unlink_job":
            result = await self.job_services.unlink_job_from_changelist(
                params.changelist_id, params.job_id, effective_user=effective_user
            )
        else:
