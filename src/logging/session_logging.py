...
class SessionManager:
    ...
    def _get_public_ip(self) -> str:
        ...
        try:
            response = requests.get(
                "https://api.ipify.org", timeout=SessionConfig.REQUEST_TIMEOUT
            )
    ...
    def log_tool_call(
        ...
        Includes impersonation audit fields when present.
        """
        session_logger = self.get_session_logger(session_id)
        if not session_logger:
            logger.warning(
                f"No active session for tool call logging: {tool_data.get('tool_name', 'unknown')}"
            )
            return

        # Ensure tool_data has expected structure (including audit identity fields)
        formatted_tool_call = {
            "request_id": tool_data.get("request_id", "unknown"),
            ...
            "outcome": tool_data.get("outcome", tool_data.get("status", "unknown")),
            "actor_user": tool_data.get("actor_user", "unknown_actor"),
            "effective_user": tool_data.get("effective_user", "unknown"),
            "p4_version": tool_data.get("p4_version", "unknown"),
        }

        reason_code = tool_data.get("reason_code")
        if reason_code:
            formatted_tool_call["reason_code"] = reason_code
