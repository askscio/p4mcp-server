...
class WorkspaceServices:
    ...
    async def get_workspace(
        self, workspace_name: str, effective_user: str | None = None
    ) -> Dict[str, Any]:
    ...
    async def list_workspaces(
        self, user: str = "", limit: int = 100, effective_user: str | None = None
    ) -> List[Dict[str, Any]]:
        ...
        async with self.connection_manager.get_connection(
            ...
            try:
                ...
                return {
                    "status": "success",
                    "message": [
                        {k: v for k, v in ws.items() if isinstance(v, str)}
    ...
    async def get_workspace_status(
        ...
        async with self.connection_manager.get_connection(
            ...
            try:
                ...
                status = {
                    "opened_files": [f["depotFile"] for f in opened_files]
                    if opened_files
                    else [],
                    "out_of_sync_files": [
                        f["depotFile"]
                        for f in out_of_sync
                        if isinstance(f, dict) and "depotFile" in f
                    ]
                    if out_of_sync
                    else [],
                    "sync_warnings": [f for f in out_of_sync if isinstance(f, str)]
                    if out_of_sync
                    else [],
                    "pending_resolves": [f["fromFile"] for f in pending_resolves]
                    if pending_resolves
                    else [],
    ...
    async def sync_workspace(
        self, path: str, force: bool, effective_user: str | None = None
    ) -> Dict[str, Any]:
    ...
    async def create_workspace(
        self, workspace_spec: Dict[str, Any], effective_user: str | None = None
    ) -> Dict[str, Any]:
        ...
        async with self.connection_manager.get_connection(
            ...
            try:
                ...
                if client_owner == current_user:
                    ...
                    return {
                        "status": "success",
                        "message": f"Workspace '{workspace_name}' owned by '{current_user}' updated successfully",
                    }
                else:
                    return {
                        "status": "failed",
                        "message": f"Workspace '{workspace_name}' is owned by '{client_owner}', not  '{current_user}'",
                    }
            except P4Exception as e:
                logger.error(
                    f"P4Error: Failed to update workspace '{workspace_name}': {e}"
                )
                return {"status": "error", "message": str(e)}
        ...
        async with self.connection_manager.get_connection(
            ...
            try:
                ...
                if client_owner == current_user:
                    ...
                    return {
                        "status": "success",
                        "message": f"Workspace '{workspace_name}' owned by '{current_user}' deleted successfully",
                    }
                else:
                    return {
                        "status": "failed",
                ...
                logger.error(
                    f"P4Error: Failed to delete workspace '{workspace_name}': {e}"
                )
                return {"status": "error", "message": str(e)}
        ...
        async with self.connection_manager.get_connection(
            ...
            try:
                ...
                if client_owner == current_user:
                    ...
                    return {
                        "status": "success",
                        "message": f"Switched to workspace '{workspace_name}' owned by '{current_user}'",
                    }
                else:
                    return {
                        "status": "failed",
                ...
                logger.error(
                    f"P4Error: Failed to switch to workspace '{workspace_name}': {e}"
                )
                return {"status": "error", "message": str(e)}
