...
class JobServices:
    ...
    async def list_jobs_from_changelist(
        self, changelist_id: str, limit: int = 50, effective_user: str | None = None
    ) -> Dict[str, Any]:
        ...
        async with self.connection_manager.get_connection(
            ...
            try:
                ...
                logger.error(
                    f"P4Error: Failed to get jobs for changelist '{changelist_id}': {e}"
                )
                return {
                    "status": "error",
                    "message": "Failed to get jobs for changelist",
                }

    async def get_job_details(
        self, job_id: str, effective_user: str | None = None
    ) -> Dict[str, Any]:
    ...
    async def link_job_to_changelist(
        self, changelist_id: str, job_id: str, effective_user: str | None = None
    ) -> None:
        ...
        async with self.connection_manager.get_connection(
            ...
            try:
                ...
                if job_id not in changelist._jobs:
                    ...
                    raise ValueError(
                        f"Job '{job_id}' is already linked to changelist '{changelist_id}'"
                    )
            except P4Exception as e:
                logger.error(
                    f"P4Error: Failed to link changelist '{changelist_id}' to job '{job_id}': {e}"
                )
                return {"status": "error", "message": str(e)}

    async def unlink_job_from_changelist(
        self, changelist_id: str, job_id: str, effective_user: str | None = None
    ) -> None:
        ...
        async with self.connection_manager.get_connection(
            ...
            try:
                ...
                if "Jobs" in changelist and job_id in changelist._jobs:
                    ...
                    raise ValueError(
                        f"Job '{job_id}' is not linked to changelist '{changelist_id}'"
                    )

            except P4Exception as e:
                logger.error(
                    f"P4Error: Failed to unlink changelist '{changelist_id}' from job '{job_id}': {e}"
                )
                return {"status": "error", "message": str(e)}
