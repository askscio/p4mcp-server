...
class ChangelistServices:
        ...
        async with self.connection_manager.get_connection(
            ...
            try:
                if changelist_id and changelist_id == "default":
                    ...
                    return {
                        "status": "success",
                        "message": {"opened_files": opened_files},
                    }
                ...
                return {
                    "status": "success",
                    "message": {k: v for k, v in changelist[0].items()},
                }
            except P4Exception as e:
                logger.error(
                    f"P4Error: Failed to get changelist '{changelist_id}': {e}"
                )
                return {
                    "status": "error",
                    "message": f"Failed to get changelist '{changelist_id}': {e}",
                }
    ...
    async def create_changelist(
        self, description: str, effective_user: str | None = None
    ) -> Dict[str, Any]:
    ...
    async def update_changelist(
        self, changelist_id: str, description: str, effective_user: str | None = None
    ) -> List[str]:
        ...
        async with self.connection_manager.get_connection(
            ...
            try:
                if not await self.verify_changelist(p4, changelist_id):
                    raise ValueError(
                        f"Changelist '{changelist_id}' does not exist or is not valid for update"
                    )
                ...
                logger.error(
                    f"P4Error: Failed to update changelist '{changelist_id}': {e}"
                )
                return {
                    "status": "error",
                    "message": f"Failed to update changelist '{changelist_id}': {e}",
                }
    ...
    async def submit_changelist(
        ...
        async with self.connection_manager.get_connection(
            ...
            try:
                if not await self.verify_changelist(p4, changelist_id):
                    raise ValueError(
                        f"Changelist '{changelist_id}' does not exist or is not valid for submit"
                    )

                submit_result = p4.run_submit("-c", changelist_id)
                return {
                    "status": "success",
                    "message": {k: v for k, v in submit_result[0].items()},
                }

            except P4Exception as e:
                logger.error(
                    f"P4Error: Failed to submit changelist '{changelist_id}': {e}"
                )
                return {"status": "error", "message": str(e)}
    ...
    async def delete_changelist(
        ...
        async with self.connection_manager.get_connection(
            ...
            try:
                if not await self.verify_changelist(p4, changelist_id):
                    raise ValueError(
                        f"Changelist '{changelist_id}' does not exist or is not valid for delete"
                    )
                ...
                if open_files:
                    raise ValueError(
                        f"Cannot delete changelist '{changelist_id}': it contains open files"
                    )
                result = p4.run("change", "-d", changelist_id)
                return {"status": "success", "message": result}
            except P4Exception as e:
                logger.error(
                    f"P4Error: Failed to delete changelist '{changelist_id}': {e}"
                )
                return {"status": "error", "message": str(e)}

    async def move_files_to_changelist(
        ...
        async with self.connection_manager.get_connection(
            ...
            try:
                if not await self.verify_changelist(p4, changelist_id):
                    raise ValueError(
                        f"Changelist '{changelist_id}' does not exist or is not valid for move files"
                    )
                if not files or not isinstance(files, list):
                    raise ValueError("No files provided to move to changelist")
                result = p4.run("reopen", "-c", changelist_id, *files)
                return {"status": "success", "message": result}
            except P4Exception as e:
                logger.error(
                    f"P4Error: Failed to move files to changelist '{changelist_id}': {e}"
                )
                return {"status": "error", "message": str(e)}
        ...
        try:
            ...
            if changelist["user"] != p4.run("info")[0]["userName"]:
                raise PermissionError(
                    f"Cannot update changelist '{changelist_id}': not owned by current user"
                )
            if changelist["status"] == "submitted":
                raise ValueError(
                    f"Cannot update submitted changelist '{changelist_id}'"
                )
            return True
        except P4Exception as e:
